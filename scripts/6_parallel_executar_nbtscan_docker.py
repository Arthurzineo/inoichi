...
def salvar_lista(ips):
    with open(TMP_LIST, "w") as f:
        for ip in sorted(ips):
            f.write(ip + "\n")

def executar_parallel():
    print("⚙️ Executando nbtscan com parallel + docker...")
    cmd = f"parallel -j 10 'python3 6_executar_nbtscan_docker.py {{}}' :::: {TMP_LIST}"
...
def main():
    ...
    ips = buscar_ips_para_nbtscan()

    if not ips:
        print("⚠️ Nenhum IP disponível para nbtscan.")
        return
